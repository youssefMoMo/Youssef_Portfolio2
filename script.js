...
const MusicPlayer = {
    audio: null, isPlaying: false, storageKey: 'yd_music_prefs',
    init() {
        ...
        // Autoplay if not disabled
        if (!prefs.musicDisabled) {
            this.autoPlay();
        } else {
            this.updateButton();
        }
    ...
    getPrefs() {
        ...
        return { musicDisabled: false, volume: 50 };
    },
    savePrefs(p) {
        try {
    ...
    autoPlay() {
        ...
        if (play) {
            play.then(() => { this.isPlaying = true; this.updateButton(); })
                .catch(() => {
                    // Wait for user interaction
                    const tryOnce = () => { if (this.getPrefs().musicDisabled || this.isPlaying) return; this.audio.play().then(() => { this.isPlaying = true; this.updateButton(); }).catch(() => { }); };
    ...
    toggle() {
        ...
        if (this.isPlaying) {
            ...
            this.savePrefs({ musicDisabled: true });
        } else {
            this.audio.play().then(() => {
                this.isPlaying = true;
                this.savePrefs({ musicDisabled: false });
            }).catch(() => { });
        }
        this.updateButton();
    },
    setVolume(v) {
        if (!this.audio) return;
        const vol = Math.max(0, Math.min(100, parseInt(v) || 50));
        this.audio.volume = vol / 100;
        this.savePrefs({ volume: vol });
        const valueEl = document.getElementById('volumeValue');
        if (valueEl) valueEl.textContent = vol + '%';
    },
    updateButton() { const b = document.querySelector('.music-btn i'); if (b) b.className = this.isPlaying ? 'fas fa-pause' : 'fas fa-play'; }
};
